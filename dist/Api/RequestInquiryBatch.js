...
class RequestInquiryBatch {
    ...
    async get(userId, monetaryAccountId, requestInquiryId) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.get(`/v1/user/${userId}/monetary-account/${monetaryAccountId}/request-inquiry-batch/${requestInquiryId}`, {}, {}, axiosClient));
    ...
    async list(userId, monetaryAccountId, options = {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.get(`/v1/user/${userId}/monetary-account/${monetaryAccountId}/request-inquiry-batch`, {}, {
            axiosOptions: {
                params: params
            }
        }, axiosClient));
    ...
    async post(userId, monetaryAccountId, requestInquiries, status = false, options = {}) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.post(`/v1/user/${userId}/monetary-account/${monetaryAccountId}/request-inquiry-batch`, requestData, {}, {}, axiosClient));
        return response.Response;
    }
    /**
     * @param {number} userId
    ...
    async put(userId, monetaryAccountId, requestInquiryId, status = "REVOKED") {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.put(`/v1/user/${userId}/monetary-account/${monetaryAccountId}/request-inquiry-batch/${requestInquiryId}`, {
            status: status
        }, {}, {}, axiosClient));
        return response.Response;
    }
