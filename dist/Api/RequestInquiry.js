...
class RequestInquiry {
    ...
    async get(userId, monetaryAccountId, requestInquiryId) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.get(`/v1/user/${userId}/monetary-account/${monetaryAccountId}/request-inquiry/${requestInquiryId}`, {}, {}, axiosClient));
    ...
    async list(userId, monetaryAccountId, options = {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.get(`/v1/user/${userId}/monetary-account/${monetaryAccountId}/request-inquiry`, {}, {
            axiosOptions: {
                params: params
            }
        }, axiosClient));
    ...
    async post(userId, monetaryAccountId, description, amount_inquired, counterpartyAlias, options = {}) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.post(`/v1/user/${userId}/monetary-account/${monetaryAccountId}/request-inquiry`, requestOptions, {}, {}, axiosClient));
        return response.Response;
    }
    /**
     * @param {number} userId
    ...
    async put(userId, monetaryAccountId, requestInquiryId, status = "REVOKED") {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.put(`/v1/user/${userId}/monetary-account/${monetaryAccountId}/request-inquiry/${requestInquiryId}`, {
            status: status
        }, {}, {}, axiosClient));
        return response.Response;
    }
