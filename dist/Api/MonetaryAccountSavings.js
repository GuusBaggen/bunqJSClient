...
class MonetaryAccountSavings {
    ...
    async get(userId, monetaryAccountSavingsId, options = {}) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.get(`/v1/user/${userId}/monetary-account-savings/${monetaryAccountSavingsId}`, {}, {}, axiosClient));
    ...
    async list(userId, options = {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.get(`/v1/user/${userId}/monetary-account-savings`, {}, {}, axiosClient));
    ...
    async post(userId, currency, description, dailyLimit, color, savingsGoal, options = {}) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.post(`/v1/user/${userId}/monetary-account-savings`, requestBody, {}, {}, axiosClient));
        return response.Response;
    }
    /**
     * @param {number} userId
    ...
    async put(userId, accountId, monetaryAccountPutRequest, options = {}) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.put(`/v1/user/${userId}/monetary-account-savings/${accountId}`, monetaryAccountPutRequest, {}, {}, axiosClient));
        return response.Response;
    }
    /**
     * @param {number} userId
    ...
    async putCancel(userId, accountId, status, sub_status, reason, options = {}) {
        const limiter = this.ApiAdapter.RequestLimitFactory.create("/monetary-account-savings", "PUT");
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.put(`/v1/user/${userId}/monetary-account-savings/${accountId}`, {
            status: status,
