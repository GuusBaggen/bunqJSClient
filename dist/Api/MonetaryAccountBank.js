...
class MonetaryAccountBank {
    ...
    async get(userId, monetaryAccountBankId, options = {}) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.get(`/v1/user/${userId}/monetary-account-bank/${monetaryAccountBankId}`, {}, {}, axiosClient));
    ...
    async post(userId, currency, description, dailyLimit, color, options = {}) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.post(`/v1/user/${userId}/monetary-account-bank`, {
            currency: currency,
    ...
    async put(userId, accountId, monetaryAccountPutRequest, options = {}) {
        ...
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.put(`/v1/user/${userId}/monetary-account-bank/${accountId}`, monetaryAccountPutRequest, {}, {}, axiosClient));
        return response.Response;
    }
    /**
     * @param {number} userId
    ...
    async putCancel(userId, accountId, status, sub_status, reason, options = {}) {
        const limiter = this.ApiAdapter.RequestLimitFactory.create("/monetary-account-bank", "PUT");
        const response = await limiter.run(async (axiosClient) => this.ApiAdapter.put(`/v1/user/${userId}/monetary-account-bank/${accountId}`, {
            status: status,
